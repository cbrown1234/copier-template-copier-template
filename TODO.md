...
- [x] Improve meta_template test infrastructure
...
- [x] Add minimum Python version as a copier question
...
- [x] Fix `ruff.toml` `target-version` to match minimum Python version
...
- [x] Change tests to check proper structure not just existence of string
      e.g. yaml key = x
- [x] Review copiers test suite for any other useful features
    - `build_file_tree` is the main utility not in our helpers, but it's only useful when constructing ad-hoc template source trees inline — not applicable when testing a single on-disk template
